...
function randomGenerator() {
  while (status == 0) {
    ...
    if (rand !== questions.length) {
      // 0.97 * 10 -> 9.7 -> 10
      // run through record array to find if it's unique
      for (var j = 0; j < record.length; j++) {
        if (rand === record[j]) {
          break; // duplicated
        } else if (j == record.length - 1) {
          // looped to the end of record array and no duplicated
          record[questionCount] = rand; // put the newly created random number to record array
